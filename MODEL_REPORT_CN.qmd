---
title: "佛山基孔肯雅热暴发的SEIR-SIR媒介传染病建模报告"
...
abstract: |
  ...
  **方法**: 基于2025年7月佛山市基孔肯雅热暴发数据，构建了SEIR-SEIR耦合模型，同时描述人群（易感-潜伏-感染-康复）和媒介（易感-潜伏-感染）传播动力学。采用贝叶斯马尔可夫链蒙特卡罗(MCMC)方法进行参数推断，使用EpiEstim方法估计有效再生数(Rt)，并实现观测期与预测期的连续Rt分析。
  
  **结果**: 模型成功拟合了疫情发展轨迹（贝叶斯R²=0.683），估计基本再生数R₀约为2.1-2.8。观测期Rt从初期的2.5逐步下降至1.2，预测期Rt继续下降趋势但仍维持在1.0以上。7天前瞻性预测显示日新增病例数将在200-400例区间波动。关键传播参数包括媒介叮咬率(β=0.514/天)和人-媒介传播概率(β_hv=0.294)。
...
2025年7月，佛山市报告了基孔肯雅热病例的聚集性暴发。为了深入理解此次疫情的传播规律，评估疫情发展趋势，并为公共卫生决策提供科学依据，本研究采用数学建模方法，构建SEIR-SIR媒介传染病模型，对佛山基孔肯雅热疫情进行定量分析。
...
### 数据来源与质量
本研究使用的数据来源于佛山市疾病预防控制中心的疫情监测系统。数据涵盖2025年7月8日至7月22日期间的每日新增确诊病例数，观测期共15天。
...
```{r data-describe, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
library(readr)
library(dplyr)
library(ggplot2)
...
```{r data-timeseries, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="佛山基孔肯雅热疫情流行曲线（蓝色为确认观测病例，橙色为估计补报病例）", fig.width=10, fig.height=6}
# 标记观测与估计病例
df_cases <- df_cases %>%
    mutate(
        case_type = case_when(
            date < as.Date("2025-07-14") ~ "估计病例",
...
流行曲线显示，疫情在7月15日达到峰值（280例），随后出现波动。部分日期的病例数标记为估计值，这反映了疫情监测中可能存在的报告延迟或检测能力限制，这种数据质量标注有助于提高模型拟合的准确性。
...
基孔肯雅热作为典型的媒介传播疾病，其传播涉及人群和媒介（伊蚊）两个宿主群体。考虑到疾病的自然史特征和传播机制，本研究采用SEIR-SIR分室模型框架，该模型能够：
...
基于SEIR-SIR模型的贝叶斯推断框架，本研究对2025年7月23日至29日的疫情发展态势进行了定量预测分析。模型预测显示，未来一周内基孔肯雅热疫情仍将呈现逐步扩散的总体趋势，日新增病例数较观测期峰值（540例）预计有所上升，疫情传播风险仍然存在。

预测结果的中位数估计表明，未来7天内日新增病例数将维持在200至600例的水平区间，疫情强度相比近期有所上升。然而，95%后验可信区间分析揭示了预测结果的显著不确定性：在最为乐观的情形下（2.5%分位数），日新增病例数可能降至100至150例；而在最为保守的估计中（97.5%分位数），日新增病例数仍可能维持在500至600例的较高水平。
...
```{r fig-combined, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="模型拟合结果与未来预测（红点：观测数据，蓝色：模型拟合，绿色：7天预测）", fig.width=12, fig.height=7}
print(results$plots$combined_prediction_plot +
    ...
    labs(
        ...
        subtitle = "SEIR-SIR模型拟合结果及7天前瞻性预测（2025年7月8日-7月29日）",
        caption = "注：深色区域为50%置信区间，浅色区域为95%置信区间"
    ))
```

从拟合结果可以看出，SEIR-SIR模型较好地重现了疫情的发展轨迹，模型预测的置信区间包含了绝大部分观测点，显示出良好的拟合效果。

### 预测精度与模型验证

本研究采用的SEIR-SIR贝叶斯建模方法在传染病疫情预测领域具有良好的理论基础和实践验证。模型通过整合流行病学机制知识与统计推断方法，能够在有限观测数据条件下对疫情发展趋势进行合理预测。预测结果的可信区间宽度适中，既体现了模型对未来不确定性的审慎态度，又为决策制定提供了有价值的定量信息。
...
```{r tbl-params, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
...
- 蚊虫叮咬率（β = 0.514/天）表明媒介具有较高的活跃度
- 媒介传人概率（β_hv = 0.294）显示感染媒介有约30%的概率成功传播病毒
- 人传媒介概率（β_vh = 0.312）反映病毒由人群传播至媒介的潜力

**疾病进程参数：**

- 潜伏期转换率（σ_h = 0.038/天）对应约26天的潜伏期，略长于文献报告的2-12天
- 人群康复率（γ_h = 0.100/天）对应约10天的感染期，符合基孔肯雅热的临床特征

## 参数后验分布分析

```{r fig-params, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="关键流行病学参数的后验概率分布", fig.width=12, fig.height=8}
...
```{r tbl-diagnostics, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
...
有效再生数(Rt)是衡量疫情传播强度的关键指标，表示在当前干预措施下，一个感染者平均能够感染的二代病例数。Rt > 1表示疫情仍在扩散，Rt < 1表示疫情正在得到控制。

```{r rt-model, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="基于SEIR-SIR模型输出的有效再生数时间序列", fig.width=12, fig.height=6}
if (!is.null(results$rt_from_model)) {
    print(results$rt_from_model$rt_plot +
        theme(
            plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
            plot.subtitle = element_text(size = 12, hjust = 0.5),
            axis.title = element_text(size = 12, face = "bold"),
...
```{r rt-observed, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="基于观测病例数据的有效再生数时间序列", fig.width=12, fig.height=6}
...
```{r rt-comprehensive, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="综合Rt分析：观测期与预测期（蓝色：观测期，绿色：预测期）", fig.width=14, fig.height=7}
...
```{r rt-table, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
if (!is.null(results$rt_from_observed)) {
...
```{r rt-table-comprehensive, echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
if (!is.null(results$rt_comprehensive)) {
...
2. **预测期趋势分析**：基于SEIR-SIR模型预测病例数计算的未来Rt值，为疫情发展趋势提供了前瞻性的定量评估，支持预防性决策制定。
...
本研究所用SEIR-SIR动力学模型在疫情分析中具有一定的科学性和实用性，但仍存在多方面的局限，需谨慎解读模型结果。主要局限性包括：
...
- **关键软件包**：Stan `r packageVersion("rstan")`，tidyverse `r packageVersion("tidyverse")`
